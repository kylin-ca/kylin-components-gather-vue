...
export default {
    ...
    methods: {
        readyNodeListPush(node) {
            let index = null
            this.readyNodeList.map((v, i) => {
                if (v.id === node.id) {
                    index = i
                }
            })
            if (index) {
                this.readyNodeList.splice(index, 1)
            } else {
                this.readyNodeList.push(node)
            }
        },
        verifyProps() {
            warning(
